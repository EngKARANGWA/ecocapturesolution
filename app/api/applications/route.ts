...
async function seedIfEmpty() {
  ...
  if (Number(rows[0].count) > 0) {
    // Backfill opening_id for any existing rows that are missing it
    for (const a of SEED) {
      await pool.query(
        `UPDATE applications SET opening_id = $1 WHERE id = $2 AND opening_id IS NULL`,
        [a.openingId, a.id]
      );
    }
    return;
