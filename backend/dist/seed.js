...
async function seedInitialData() {
    const { rows: oRows } = await db_1.pool.query('SELECT COUNT(*) FROM openings');
    if (Number(oRows[0].count) === 0) {
        for (const o of OPENINGS) {
            await db_1.pool.query(`INSERT INTO openings (id, title, type, location, description, tags, status)
         VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (id) DO NOTHING`, [o.id, o.title, o.type, o.location, o.description, JSON.stringify(o.tags), o.status]);
        }
        console.log('Openings seeded');
    }
    const { rows: pRows } = await db_1.pool.query('SELECT COUNT(*) FROM partners');
    if (Number(pRows[0].count) === 0) {
        for (const p of PARTNERS) {
            await db_1.pool.query(`INSERT INTO partners (id, name, logo, website, type, status)
         VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (id) DO NOTHING`, [p.id, p.name, p.logo, p.website, p.type, p.status]);
        }
        console.log('Partners seeded');
    }
    const { rows: aRows } = await db_1.pool.query('SELECT COUNT(*) FROM applications');
    if (Number(aRows[0].count) === 0) {
        for (const a of APPLICATIONS) {
            await db_1.pool.query(`INSERT INTO applications (id, opening_id, data, status) VALUES ($1,$2,$3,$4) ON CONFLICT (id) DO NOTHING`, [a.id, a.openingId, JSON.stringify(a.data), a.status]);
        }
        console.log('Applications seeded');
    }
    const { rows: fRows } = await db_1.pool.query('SELECT COUNT(*) FROM form_schemas');
    ...
    const { rows: statRows } = await db_1.pool.query('SELECT COUNT(*) FROM site_stats');
    if (Number(statRows[0].count) === 0) {
        for (const stat of SITE_STATS) {
            await db_1.pool.query(`INSERT INTO site_stats (key, label, value, suffix, description, status, sort_order)
         VALUES ($1,$2,$3,$4,$5,'active',$6) ON CONFLICT (key) DO NOTHING`, [stat.key, stat.label, stat.value, stat.suffix ?? null, stat.description ?? null, stat.sort_order]);
        }
        console.log('Site stats seeded');
    }
    const { rows: productRows } = await db_1.pool.query('SELECT COUNT(*) FROM products');
    if (Number(productRows[0].count) === 0) {
        for (const product of PRODUCTS) {
            await db_1.pool.query(`INSERT INTO products (id, name, slug, summary, features, image, category, status, sort_order)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT (id) DO NOTHING`, [product.id, product.name, product.slug, product.summary, JSON.stringify(product.features), product.image, product.category, product.status, product.sort_order]);
        }
        console.log('Products seeded');
    }
    const { rows: storyRows } = await db_1.pool.query('SELECT COUNT(*) FROM company_story');
    if (Number(storyRows[0].count) === 0) {
        for (const [index, paragraph] of COMPANY_STORY.entries()) {
            await db_1.pool.query(`INSERT INTO company_story (id, paragraph, status, sort_order)
         VALUES ($1,$2,'active',$3) ON CONFLICT (id) DO NOTHING`, [`story-${index + 1}`, paragraph, index + 1]);
        }
        console.log('Company story seeded');
    }
    const { rows: newsRows } = await db_1.pool.query('SELECT COUNT(*) FROM news_items');
    if (Number(newsRows[0].count) === 0) {
        for (const news of NEWS) {
            await db_1.pool.query(`INSERT INTO news_items (id, title, body, category, publish_date, featured, archived, slug)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT (id) DO NOTHING`, [news.id, news.title, news.body, news.category, news.publish_date, news.featured, news.archived, news.slug]);
        }
        console.log('News seeded');
    }
    const { rows: projectRows } = await db_1.pool.query('SELECT COUNT(*) FROM projects');
    if (Number(projectRows[0].count) === 0) {
        for (const project of PROJECTS) {
            await db_1.pool.query(`INSERT INTO projects (id, title, description, location, status, timeline, partners, results, images)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT (id) DO NOTHING`, [project.id, project.title, project.description, project.location, project.status, project.timeline, JSON.stringify(project.partners), JSON.stringify(project.results), JSON.stringify(project.images)]);
        }
        console.log('Projects seeded');
    }
    const { rows: testimonialRows } = await db_1.pool.query('SELECT COUNT(*) FROM testimonials');
    if (Number(testimonialRows[0].count) === 0) {
        for (const testimonial of TESTIMONIALS) {
            await db_1.pool.query(`INSERT INTO testimonials (id, name, role, quote, category, status)
         VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (id) DO NOTHING`, [testimonial.id, testimonial.name, testimonial.role, testimonial.quote, testimonial.category, testimonial.status]);
        }
        console.log('Testimonials seeded');
    }
    const { rows: resourceRows } = await db_1.pool.query('SELECT COUNT(*) FROM resources');
    if (Number(resourceRows[0].count) === 0) {
        for (const resource of RESOURCES) {
            await db_1.pool.query(`INSERT INTO resources (id, title, description, file_url, type, category)
         VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (id) DO NOTHING`, [resource.id, resource.title, resource.description, resource.file_url, resource.type, resource.category]);
        }
